...
Used CDP network capture (`browser.navigate` + `browser.eval` fetch interceptor) on `ubereats.com/orders`.
...
See `docs/src/content/docs/apps/overview.md` for `http.headers()` documentation.
...
#### `getReceiptByWorkflowUuidV1` — receipt with item details (discovered 2026-04-02; JSON path 2026-07-15)
...
**Prefer `contentType: "JSON"`** (2026-07-15) — `receiptData` is a JSON string with
`cart[].Items[{Title, Quantity, UnitPrice.AmountE5, TotalPrice, Uuid, Customizations}]`
and `fare.amount_charged`. AmountE5 ÷ 1e5 → dollars. Parsed in shipped
`page/eats.js` (`__ueats._parseReceiptJson`). HTML below is the fallback only.
...
**Item extraction from receipt HTML (confirmed 2026-04-02; fallback):**
...
Parse with lxml + cssselect (per app standards — no BS4, no regex on HTML):
```python
