...
Read-only macOS inspection app for local computer awareness. This first pass is intentionally limited to discovery and screenshots only.
...
```javascript
run({ app: "macos-control", tool: "list_apps", params: { limit: 10 } })
```
...
```javascript
run({ app: "macos-control", tool: "list_processes", params: { limit: 25 } })
```
...
```javascript
run({ app: "macos-control", tool: "list_displays" })
```
...
```javascript
run({ app: "macos-control", tool: "list_windows", params: { limit: 20 } })
```
...
```javascript
run({ app: "macos-control", tool: "screenshot_window", params: { window_id: 12345 } })
```
...
```javascript
run({ app: "macos-control", tool: "screenshot_display", params: { display_index: 1 } })
```

## Scope

This app currently does not mutate app or window state. It does not open, focus, move, resize, quit, or force-quit anything in this first pass.
